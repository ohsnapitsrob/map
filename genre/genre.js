(function () {
  ...
  async function fetchCSV(url) { if (!url) return []; const response = await fetch(url, { cache: "no-store" }); if (!response.ok) throw new Error(`Could not load CSV: ${url}`); return rowsToObjects(parseCSV(await response.text())); }
